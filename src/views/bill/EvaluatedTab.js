...
const EvaluatedTab = (route) => {
    const navigation = useNavigation();
    const dispatch = useDispatch();
    const bills = useSelector(selectEvaluatedBills);
    const [extraBills, setextraBills] = useState([]);
    const [isLoading, setisLoading] = useState(true);
    const [isRefreshing, setisRefreshing] = useState(false);

    async function onGetBills() {
        dispatch(fetchEvaluatedBills());
    }
                            ...
                            (bills && bills.length > 0)
                                ...
                                : <ScrollView refreshControl={
                                    ...
                                    <View style={styles.viewEmptyList}>
                                        ...
                                        <Text style={styles.textEmptyList}>Không có đơn hàng đã đánh giá..</Text>
                                    </View>
                                </ScrollView>
                        }
                    </>
...
export default memo(EvaluatedTab);