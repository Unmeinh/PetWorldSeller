...
import { onAxiosGet, onAxiosPut } from '../../api/axios.function';
import { onGoBack } from '../../navigation/rootNavigation';
import { useNavigation } from '@react-navigation/native';
import { useDispatch } from 'react-redux';
import { updateProduct } from '../../redux/reducers/product/productReducer';

const EditProduct = ({ route }) => {
    const navigation = useNavigation();
    const dispatch = useDispatch();
    let oldProduct = route.params.product;
    const [arr_Image, setarr_Image] = useState((oldProduct.arrProduct) ? oldProduct.arrProduct : []);
    const [arr_Category, setarr_Category] = useState([]);
    const [selectCategory, setselectCategory] = useState([]);
    const [oldCategory, setoldCategory] = useState("");
    const [inputNameProduct, setinputNameProduct] = useState((oldProduct.nameProduct) ? oldProduct.nameProduct : "");
    const [inputCategory, setinputCategory] = useState("");
    const [inputPrice, setinputPrice] = useState((oldProduct.priceProduct) ? Number(oldProduct.priceProduct).toLocaleString() : "");
    const [inputDiscount, setinputDiscount] = useState((oldProduct.discount) ? String(oldProduct.discount) : "0");
    const [inputAmount, setinputAmount] = useState((oldProduct.amountProduct) ? String(oldProduct.amountProduct) : "");
    const [inputDetail, setinputDetail] = useState((oldProduct.detailProduct) ? oldProduct.detailProduct : "");
    ...
    async function onEditProduct() {
        ...
        formData.append("id", oldProduct._id);
    ...
    React.useEffect(() => {
        if (arr_Category.length > 0) {
            ...
            if (oldProduct.idCategoryPr) {
                for (let i = 0; i < arr_Category.length; i++) {
                    const element = arr_Category[i];
                    if (oldProduct.idCategoryPr?._id == element?._id) {
                        setoldCategory(i);
