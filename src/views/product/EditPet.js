...
import { onAxiosPut, onAxiosGet } from '../../api/axios.function';
import { onGoBack } from '../../navigation/rootNavigation';
import { useNavigation } from '@react-navigation/native';
import { convertInputToFloat } from '../../utils/functionSupport';
import { useDispatch } from 'react-redux';
import { updatePet } from '../../redux/reducers/pet/petReducer';

const EditPet = ({ route }) => {
    const navigation = useNavigation();
    const dispatch = useDispatch();
    const oldPet = route.params.pet;
    const [arr_Image, setarr_Image] = useState((oldPet.imagesPet) ? oldPet.imagesPet : []);
    const [arr_Category, setarr_Category] = useState([]);
    const [selectCategory, setselectCategory] = useState([]);
    const [oldCategory, setoldCategory] = useState("");
    const [inputNamePet, setinputNamePet] = useState((oldPet.namePet) ? oldPet.namePet : "");
    const [inputWeight, setinputWeight] = useState((oldPet.weightPet) ? String(oldPet.weightPet).replaceAll('.', ',') : "");
    const [inputHeight, setinputHeight] = useState((oldPet.heightPet) ? String(oldPet.heightPet).replaceAll('.', ',') : "");
    const [inputSize, setinputSize] = useState("0");
    const [inputCategory, setinputCategory] = useState("");
    const [inputPrice, setinputPrice] = useState((oldPet.pricePet) ? Number(oldPet.pricePet).toLocaleString() : "");
    const [inputDiscount, setinputDiscount] = useState((oldPet.discount) ? String(oldPet.discount) : "0");
    const [inputAmount, setinputAmount] = useState((oldPet.amountPet) ? String(oldPet.amountPet) : "");
    const [inputDetail, setinputDetail] = useState((oldPet.detailPet) ? oldPet.detailPet : "");
    ...
    async function onEditPet() {
        Toast.show({
            type: 'loading',
            text1: 'Đang sửa thú cưng...',
            position: 'top',
            autoHide: false
        })

        var formData = new FormData();
        formData.append("id", oldPet._id);
        ...
        let res = await onAxiosPut('pet/update', formData, 'formdata', true);
        if (res) {
            dispatch(updatePet([oldPet._id, res.data]));
            onGoBack();
        }
    }
    ...
    React.useEffect(() => {
        if (arr_Category.length > 0) {
            ...
            if (oldPet.idCategoryP) {
                for (let i = 0; i < arr_Category.length; i++) {
                    const element = arr_Category[i];
                    if (oldPet.idCategoryP?._id == element?._id) {
                        setoldCategory(i);
                        setinputCategory(element?._id);
                    }
                }
            }
        }
    ...
    return (
        <View style={styles.container}>
            <HeaderTitle colorHeader={yellowWhite} titleHeader={"Sửa thú cưng"} />
                        ...
                        <View style={{ width: '47%', }}>
                            ...
                            <View>
                                <SelectDropdown
                                    ...
                                    defaultValueByIndex={oldCategory}
                                    buttonStyle={[{ width: '100%' }, styles.textInputLogin, styles.bgLightBrown]}
                                    buttonTextStyle={[styles.textDarkBlue, { fontSize: 17 }]}
                                    onSelect={(selectedItem, index) => {
